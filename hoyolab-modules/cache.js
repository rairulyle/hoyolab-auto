module.exports = class DataCache {
	...
	async #updateCachedData(cachedData) {
		...
		if (cachedData.stamina) {
			...
			cachedData.stamina.recoveryTime = Math.max(
				0,
				cachedData.stamina.recoveryTime - secondsSinceLastUpdate
			);
			cachedData.stamina.currentStamina =
				Math.round(cachedData.stamina.currentStamina * 100) / 100;
			...
			const staminaAlmostFull =
				cachedData.stamina.maxStamina - cachedData.stamina.currentStamina <= 10 &&
				isAboveThreshold;
	...
	#shouldInvalidateExpedition(cachedData, secondsSinceLastUpdate) {
		...
		for (const expedition of cachedData.expedition.list) {
			expedition.remaining_time = Math.max(
				0,
				Number(expedition.remaining_time) - Math.round(secondsSinceLastUpdate)
			);
