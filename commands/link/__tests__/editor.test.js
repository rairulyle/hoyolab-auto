...
test("game select offers a ToT enable option only when the profile lacks it", () => {
	const values = (p) => buildGameSelect(p).components[0].components[0].options.map(o => o.data.value);
	...
	const withTot = values({ ...profile, games: [...profile.games, { key: "termis", uid: null, active: true, settings: {} }]});
	assert.equal(withTot.filter(v => v === "termis").length, 1, "should not duplicate termis when already present");
});
